...
ifeq ($(VERSION),us)
	...
	CFLAGS += -DTP_WUS0 -DPLATFORM_WII=1
	CFLAGS += -D_BUILDID='"$(BUILDID)WUS0"'
	ASFLAGS += -DTP_WUS0 -DPLATFORM_WII=1
	PACKVER = "us0"
else ifeq ($(VERSION),wus2)
	CFLAGS += -DTP_WUS2 -DPLATFORM_WII=1
	CFLAGS += -D_BUILDID='"$(BUILDID)WUS2"'
	ASFLAGS += -DTP_WUS2 -DPLATFORM_WII=1
	PACKVER = "us2"
else ifeq ($(VERSION),weu)
	CFLAGS += -DTP_WEU -DPLATFORM_WII=1
	CFLAGS += -D_BUILDID='"$(BUILDID)WEU"'
	ASFLAGS += -DTP_WEU -DPLATFORM_WII=1
	PACKVER = "eu"
else ifeq ($(VERSION),wjp)
	CFLAGS += -DTP_WJP -DPLATFORM_WII=1
	CFLAGS += -D_BUILDID='"$(BUILDID)WJP"'
	ASFLAGS += -DTP_WJP -DPLATFORM_WII=1
	PACKVER = "jp"
endif
